...
class CountMinSketch(object):
    ...
    def __init__(self, m=None, d=None, delta=None, epsilon=None):
        ...
        if m is not None and d is not None:
            self.m = m
            self.d = d
            self.delta = float(1 / (math.pow(math.e, d)))
...
if __name__ == "__main__":
    ...
    locais = ["aveiro", "braga", "viseu", "coimbra", "santarem", "leiria", "açores", "madeira", "minho"]

    # counter = CMS(5, 2)
    counter = CountMinSketch(None, None, 0.001, 0.001)

    for query in locais:
        print("---------------")
        counter.update(query, 2)
        print(counter.query(query))
    print("---------------")
